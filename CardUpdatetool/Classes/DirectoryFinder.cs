...
namespace CardUpdateTool
{
    internal static class DirectoryFinder
    {
        public static List<string> Grab_All_Directories(string originalPath)
        {
            ...
            foldersPath.AddRange(Directory.GetDirectories(originalPath, "*",
                SearchOption.AllDirectories)); //grab child folders
            for (var i = 0; i < foldersPath.Count; i++)
            {
                ...
                if (Directory.GetFiles(foldersPath[i], "*.png").Length == 0) foldersPath.RemoveAt(i--);
            }

            if (foldersPath.Count == 0) foldersPath.Add(originalPath);
        ...
        public static List<string> Get_Cards_From_Path(string originalPath)
        {
            ...
            if (Directory.Exists(originalPath))
            {
                ...
                paths.AddRange(Directory.GetDirectories(originalPath, "*",
                    SearchOption.AllDirectories)); //grab child folders
            }
            ...
            foreach (var path in paths)
            {
                if (path.Contains("BadCardData") || path.Contains("MissingMods") ||
                    path.Contains("OutdatedMods")) continue;
